    ...
    for line in fin:
        ...
        if oper == 'INCOME':
            for k, e in mydict.items():
                if int(e) > 0:
                    mydict[k] *= (1 + int(line.split()[1]) / 100)
                    mydict[k] = int(mydict[k])
        elif oper == 'BALANCE':
            user = line.split()[1]
            print(mydict.get(user, 'ERROR'))
        elif oper == 'DEPOSIT':
            user, dep = line.split()[1], int(line.split()[2])
            mydict[user] = mydict.get(user, 0) + dep
        elif oper == 'WITHDRAW':
            user, wdraw = line.split()[1], int(line.split()[2])
            mydict[user] = mydict.get(user, 0) - wdraw
        elif oper == 'TRANSFER':
            user1 = line.split()[1]
            user2 = line.split()[2]
            tran = int(line.split()[3])
            mydict[user1] = mydict.get(user1, 0) - tran
            mydict[user2] = mydict.get(user2, 0) + tran
...
# Первоначально у банка нет ни одного клиента. Как только для клиента проводится операция пополнения,
